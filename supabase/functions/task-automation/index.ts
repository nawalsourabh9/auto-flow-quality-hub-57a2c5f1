      ...
      for (const task of completedTasks) {
        try {
          ...
          if (shouldProcess) {
            ...
            // Remove the restrictive daysDifference check
            // The logic for determining the next date and checking against the implicit end date
            // should reside within the generate_next_recurring_task RPC function.

            console.log(`Processing completed recurring task: ${task.title}`);

            // Use the completed task ID (whether parent or child) to trigger next instance generation via RPC
            ...
            if (generateError) {
              ...
              console.log(`No new task generated by RPC for ${task.title} (conditions within RPC not met)`);
            }
          }
        } catch (taskError) {
