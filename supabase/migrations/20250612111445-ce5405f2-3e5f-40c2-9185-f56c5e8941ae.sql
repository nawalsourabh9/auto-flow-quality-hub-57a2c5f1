...
-- Let's examine and fix the generate_next_recurring_task function to ensure proper data types
-- The issue is likely in how we're handling date calculations or field assignments

-- First, let's check if there are any issues with the function
-- Update the function to ensure all integer fields receive proper integer values
...
DECLARE
 ...
 BEGIN
   ...
   -- Calculate days since the completed task's start date (ensure INTEGER result)
   days_since_completion := EXTRACT(DAY FROM (current_date_val - completed_task.start_date))::INTEGER;
   ...
   -- Ensure we get an INTEGER result
   SELECT COALESCE(MAX(recurrence_count_in_period), 0) + 1 
   INTO next_count
   FROM tasks 
   WHERE parent_task_id = parent_task.id;
   
   -- Ensure next_count is an INTEGER (this should prevent the "9 days" error)
   next_count := COALESCE(next_count, 1)::INTEGER;
   ...
   -- Create new task instance with explicit INTEGER casting for recurrence_count_in_period
   INSERT INTO tasks (
     title,
     description,
     department,
     ...
     next_count::INTEGER  -- Explicit INTEGER casting to prevent type errors
