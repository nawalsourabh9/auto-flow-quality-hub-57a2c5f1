...
-- Apply the latest migration: fix recurring generation to allow immediate generation
-- This removes strict time validation and allows immediate generation when a task is completed
...
BEGIN
  ...
  -- *** REMOVED STRICT TIME VALIDATION - ALLOW IMMEDIATE GENERATION ***
  -- No time restrictions - allow generation immediately when task is completed
